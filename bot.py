...
def build_system_prompt(user, db):
    ...
    return (
        ...
        "=== XP — СТРОГИЕ ПРАВИЛА ===\n"
        "XP начисляй ТОЛЬКО когда пользователь сообщает о РЕАЛЬНО ВЫПОЛНЕННОМ деле:\n"
        ...
        "- weekly: description + time + days (Mon,Wed).\n"
        "- Заполняй task, когда просят напомнить / поставить задачу.\n\n"
        "=== QUERY ===\n"
        "Если вопрос про финансы требует данных не из сводки → заполни query.\n\n"
        "Ответ — один JSON:\n"
...
def handle_parsed_response(parsed, user, db, user_text, chat_id, message_id, now):
    ...
    if set_balance is not None:
        try:
            user.balance = float(set_balance)
...
def help_command(message):
    help_text = (
        ...
        "XP даётся за реальные дела, не за болтовню.\n\n"
        "**⚙️ Настройки**\n"
